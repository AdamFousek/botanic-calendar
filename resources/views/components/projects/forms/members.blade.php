    ...
    @if ($members)
        <div class="my-2">
            @foreach($members as $user)
                <x-badge wire:click="toggleMembers({{ $user->id }})" class="cursor-pointer">
                    {{ $user->fullName ?: $user->username }}
                </x-badge>
            @endforeach
        </div>
    @endif
    <x-text-input wire:model.debounce.300ms="search" class="block mt-1 w-full" type="text" placeholder="{{ __('Search for specific member...') }}" />
